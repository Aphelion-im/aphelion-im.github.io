	...
	function display($selection = '', $group = '', $emails = ''){
		...
		$js = 'function timeoutAddNewTestAddress(currentValue){
			...
			function addNewTestAddress(currentValue){
				if(currentValue.length > 1 && (currentValue.slice(-1) == " " || currentValue.slice(-1) == ";" || currentValue.slice(-1) == ",")){
					currentValue = currentValue.substring(0, currentValue.length - 1);
					setUser(currentValue);
					return;
				}';
		if($app->isAdmin()){
			$js .= 'if(currentValue.length < 3){ document.getElementById("acymailing_divSelectReceiver").style.display = "none"; return; }
				if(currentValue != document.getElementById("message_receivers").value) return;
		...
		$js .= '}
			...
			function setUser(userEmail){
				if(validateEmail(userEmail, "") && selected.indexOf(userEmail) == -1){
