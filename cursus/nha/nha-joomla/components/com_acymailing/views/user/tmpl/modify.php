		...
		<?php if(acymailing_level(3)){
			...
			foreach($this->extraFields as $fieldName => $oneExtraField) {
				if($oneExtraField->type == 'category'){
					...
					echo '<div id="tr'.$fieldName.'"><div width="150" class="key"><label>'.$this->fieldsClass->getFieldName($oneExtraField).'</label></div>';
...
?>
	...
	<fieldset class="adminform acy_subscription_list">
		...
		<div width="100%" id="acyusersubscription">
			<div nowrap="nowrap" width="150" class="subscriptionTitle"><?php echo JText::_( 'SUBSCRIBE' );?></div>
			<?php
			$k = 0;
			foreach($this->subscription as $row){
				...
				?>
			<div class="<?php echo "row$k $listClass"; ?>">
				<div align="center" nowrap="nowrap" valign="top" class="acystatus">
					<span><?php echo $this->status->display("data[listsub][".$row->listid."][status]",@$row->status); ?></span>
				</div>
				<div valign="top" class="acyListInfo">
